...
def filter(image, mask):
    ...
    mrows, mcols = mask.shape[:2]
    dst = np.zeros((rows, cols), np.float32)
    ycenter, xcenter = mrows//2, mcols//2 #mask.shape[0] -> 행의 개수, mask.shape[1] -> 열의 개수 , row 행 col 열
...
def filter2(image, mask):
    rows, cols = image.shape[:2]
    mcols, mrows = mask.shape[:2]
    dst = np.zeros((rows, cols), np.float32)
    ycenter, xcenter = mrows//2, mcols//2
...
image = cv2.imread("ch7images/filter_blur.jpg", cv2.IMREAD_GRAYSCALE)
